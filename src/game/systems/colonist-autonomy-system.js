...
(() => {
  ...
  const SLEEP_SURFACE = Object.freeze({
    BED: 'bed',
    FLOOR: 'floor',
    ROUGH: 'rough',
    OUTDOOR: 'outdoor'
  ...
  const ENERGY = Object.freeze({
    ...
    PREPARE_SLEEP: 45,
    WAKE_WORK: 45,
    WAKE_SLEEP_WINDOW: 88,
    FULL_REST: 96
  });

  const DECISION = Object.freeze({
    MIN_INTERVAL: 0.08,
    SHORT_BEFORE_SLEEP: 0.25,
    SOON_BEFORE_SLEEP: 0.75
  });

  const original = {
    updateColonist: null,
    startSleep: null,
    handleTaskAtTarget: null
  };
  ...
  function worldTime() {
    return (Number(state?.day || 0) * 24) + Number(state?.hour || 0);
  }

  function ensureBrain(c) {
    c.brain = c.brain || {};
    c.brain.intent = c.brain.intent || INTENT.IDLE;
    c.brain.reason = c.brain.reason || 'initial';
    c.brain.lockedUntil = Number(c.brain.lockedUntil || 0);
    c.brain.lastDecisionAt = Number(c.brain.lastDecisionAt || 0);
    return c.brain;
  }

  function ensureRestState(c) {
    c.rest = c.rest || {};
    c.rest.sleptOnFloorHours = Number(c.rest.sleptOnFloorHours || 0);
    c.rest.lastSleepSurface = c.rest.lastSleepSurface || null;
    c.rest.sleepQuality = Number(c.rest.sleepQuality || 1);
    c.bodyPain = clampValue(Number(c.bodyPain || 0), 0, 100);
    return c.rest;
  }

  function rememberIntent(c, intent, reason, lockHours = 0.12) {
    const brain = ensureBrain(c);
    brain.intent = intent;
    brain.reason = reason;
    brain.lastDecisionAt = worldTime();
    brain.lockedUntil = worldTime() + Math.max(0, Number(lockHours) || 0);
    c.intent = intent;
    c.intentReason = reason;
    return brain;
  }

  function scheduleState(c, hour = state?.hour || 0) {
    return window.ScheduleManager?.getScheduleState?.(c, hour) ?? window.ScheduleManager?.SCHEDULE?.WORK ?? 1;
  }

  function isSchedule(c, scheduleValue) {
    return scheduleState(c) === scheduleValue;
  }
  ...
  function hoursUntilSchedule(c, target) {
    ...
    for (let i = 0; i <= 24; i++) {
      ...
      if (schedule[hour] === target) return Math.max(0, i - fraction);
    }
    return Infinity;
  }
  ...
  function taskType(c) {
    return c?.task?.type || null;
  }

  function isCombatTask(type) {
    return type === 'combat' || type === 'scare';
  }

  function isWorkTask(type) {
    return ['gather','mine','build','buildRoof','haul','deconstruct','research','craft','forge','cook','inspect','loot','inspectPoi','heal'].includes(type);
  }
  ...
  function pathTo(c, x, y, target = null) {
    if (!Number.isFinite(Number(x)) || !Number.isFinite(Number(y))) return [];
    if (Math.round(c.x) === Math.round(x) && Math.round(c.y) === Math.round(y)) return [];
    return typeof findPath === 'function' ? (findPath(c.x, c.y, x, y, target) || []) : [];
  }
  ...
  function tileTerrain(x, y) {
    return state?.terrain?.[y]?.[x] || state?.world?.terrain?.[y]?.[x] || 'grass';
  }

  function hasRoofAt(x, y) {
    return typeof hasNaturalRoofAt === 'function' ? !!hasNaturalRoofAt(x, y) : !!state?.world?.naturalRoofLayer?.[y]?.[x];
  }
  ...
  function sleepSurfaceAt(x, y) {
    ...
    if (state?.weather === 'chuva' && !roof) return SLEEP_SURFACE.OUTDOOR;
  ...
  function surfaceLabel(surface) {
    if (surface === SLEEP_SURFACE.BED) return 'cama';
    if (surface === SLEEP_SURFACE.FLOOR) return 'chão';
    if (surface === SLEEP_SURFACE.OUTDOOR) return 'relento';
    return 'chão duro';
  }
  ...
  function chooseSleepSurface(c, reason = 'tired') {
    ...
    return {
      ...
      reason,
      floorRest: true,
      emergency: Number(c.energy ?? 100) <= ENERGY.EMERGENCY_SLEEP
    };
  }
  ...
  function startAutonomySleep(c, reason = 'tired') {
    ...
    c.path = chosen.path || [];
    c.work = 0;
    c.note = chosen.surface === SLEEP_SURFACE.BED
      ? (c.path.length ? 'Indo dormir na cama' : 'Dormindo na cama')
      : `Dormindo no ${surfaceLabel(chosen.surface)}`;
  ...
  function wakeColonist(c, note = 'Descansado') {
    ...
    c.work = 0;
    c.note = note;
    rememberIntent(c, INTENT.IDLE, 'rested', 0.08);
  }
  ...
  function handleSleep(c, task, tick) {
    ...
    if (!scheduledSleep && c.energy >= wakeTarget) {
      wakeColonist(c, c.energy >= ENERGY.FULL_REST ? 'Totalmente descansado' : 'Descansado');
      return true;
    }

    if (scheduledSleep && c.energy >= ENERGY.FULL_REST) {
      wakeColonist(c, 'Totalmente descansado');
      return true;
    }
  ...
  function applyVitals(c, tick) {
    c.energy = clampValue(Number(c.energy ?? 100), 0, 100);
    c.hunger = clampValue(Number(c.hunger ?? 100), 0, 100);
    c.mood = clampValue(Number(c.mood ?? 70), 0, 100);
    c.health = clampValue(Number(c.health ?? 100), 1, 100);
    c.bodyPain = clampValue(Number(c.bodyPain || 0), 0, 100);
    ...
    let drain = 0.010;
    const type = taskType(c);
    if (type === 'sleep') drain = 0;
    else if (type === 'leisure') drain = -0.09;
    else if (isCombatTask(type)) drain = 0.060;
    else if (isHeavyTask(type)) drain = 0.046;
    else if (isWorkTask(type)) drain = 0.032;
    else if (type === 'move' || (Array.isArray(c.path) && c.path.length)) drain = 0.018;
  ...
  function maybeEat(c) {
    if (!state?.resources || c.hunger >= 32 || Number(state.resources.food || 0) <= 0 || c.task?.type === 'sleep') return false;
    state.resources.food -= 1;
    c.hunger = clampValue(c.hunger + 42, 0, 100);
    c.mood = clampValue(c.mood + 4, 0, 100);
    c.note = 'Comeu uma refeição rápida';
    rememberIntent(c, INTENT.EAT, 'hungry', 0.08);
    if (typeof log === 'function') log(`${c.name} comeu uma refeição rápida.`);
    return true;
  }

  function shouldStartEmergencySleep(c) {
    if (c.task?.type === 'sleep') return false;
    if (Number(c.energy ?? 100) <= ENERGY.EMERGENCY_SLEEP) return !isCombatTask(taskType(c));
    return false;
  }

  function shouldStartForcedSleep(c) {
    if (c.task?.type === 'sleep') return false;
    const energy = Number(c.energy ?? 100);
    if (energy > ENERGY.FORCE_SLEEP) return false;
    if (!interruptibleForNeed(c)) return false;
    if (shouldFinishCurrentTask(c) && energy > ENERGY.EMERGENCY_SLEEP + 3) return false;
    return true;
  }

  function shouldStartScheduledSleep(c) {
    if (c.task?.type === 'sleep') return false;
    const manager = window.ScheduleManager;
    if (!manager?.SCHEDULE || !isSchedule(c, manager.SCHEDULE.SLEEP)) return false;
    if (!interruptibleForNeed(c)) return false;
    if (shouldFinishCurrentTask(c) && Number(c.energy ?? 100) > 32) return false;
    return true;
  }

  function shouldAvoidNewLongWork(c) {
    const manager = window.ScheduleManager;
    if (!manager?.SCHEDULE) return false;
    const untilSleep = hoursUntilSchedule(c, manager.SCHEDULE.SLEEP);
    return untilSleep <= DECISION.SOON_BEFORE_SLEEP && Number(c.energy ?? 100) < 55;
  }

  function canWorkNow(c) {
    const manager = window.ScheduleManager;
    if (!manager?.SCHEDULE) return true;
    if (isSchedule(c, manager.SCHEDULE.WORK)) return true;
    if (isSchedule(c, manager.SCHEDULE.LEISURE)) {
      return workExists() && Number(c.energy ?? 100) > 32 && Number(c.mood ?? 100) > 18;
    }
    return false;
  }

  function assignLeisure(c) {
    if (window.ScheduleManager?.assignLeisure?.(c)) {
      rememberIntent(c, INTENT.LEISURE, 'schedule', 0.18);
      return true;
    }
    return false;
  }
  ...
  function applyDecision(c) {
    ...
    if (c.task?.type === 'sleep') {
      if (!isSchedule(c, schedule.SLEEP) && Number(c.energy ?? 100) >= ENERGY.WAKE_WORK) wakeColonist(c, 'Voltando ao trabalho');
      return true;
    }
    ...
    if (c.task) {
      if (isWorkTask(taskType(c))) rememberIntent(c, INTENT.WORK, 'current-task', 0.10);
      return false;
    }
    ...
    if (isSchedule(c, schedule.LEISURE)) {
      if (Number(c.energy ?? 100) <= ENERGY.FORCE_SLEEP) return startAutonomySleep(c, 'leisure-exhausted');
      if (!workExists() || Number(c.mood ?? 100) < 35) return assignLeisure(c);
    }

    if (shouldAvoidNewLongWork(c)) {
      c.note = 'Preparando descanso';
      rememberIntent(c, INTENT.IDLE, 'sleep-soon', 0.12);
      return true;
    }

    if (canWorkNow(c)) {
      rememberIntent(c, INTENT.WORK, 'available', 0.10);
      const assigned = typeof assignAutoTask === 'function' ? assignAutoTask(c) : false;
      if (assigned) return true;
    }

    if (Number(c.mood ?? 100) < 24 && Number(c.energy ?? 100) > 24) {
      if (assignLeisure(c)) return true;
    }
  ...
  function movementPainMultiplier(c) {
    const pain = Number(c?.bodyPain || 0);
    return 1 - Math.min(0.30, pain / 260);
  }

  function workPainMultiplier(rate, c, kind) {
    const pain = Number(c?.bodyPain || 0);
    let penalty = Math.min(0.30, pain / 240);
    if (isHeavyTask(kind)) penalty = Math.min(0.38, pain / 190);
  ...
  function autonomyUpdateColonist(c, dt) {
    ...
    if (c.task) {
      if (Array.isArray(c.path) && c.path.length) {
        if (typeof moveAlongPath === 'function') moveAlongPath(c, tick);
      } else if (c.task.type === 'sleep') {
        handleSleep(c, c.task, tick);
      } else if (typeof handleTaskAtTarget === 'function') {
        handleTaskAtTarget(c, tick);
      }
    }
  ...
  function install() {
    if (!window.HavenfallColonistVitals || typeof updateColonist !== 'function') {
    ...
    window.HavenfallColonistAutonomy = Object.freeze({
      INTENT,
      SLEEP_SURFACE,
      ENERGY,
      ownsDecision,
      ensureBrain,
      ensureRestState,
      startSleep: startAutonomySleep,
      chooseSleepSurface,
      original,
      version: 'autonomy-ground-sleep-v1'
    });

    console.info('[Colonist Autonomy] Cérebro individual e sono no chão carregados.');
  }
