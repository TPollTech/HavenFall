...
(() => {
  if (window.HavenfallContext?.workCoordinationV2Installed) return;
  window.HavenfallContext = window.HavenfallContext || {};
  window.HavenfallContext.workCoordinationV2Installed = true;
  ...
  function installOverrides() {
    ...
    if (!window.HavenfallContext.workCoordinationV2WanderPatched && typeof randomWander === 'function') {
      randomWander = c => calmIdle(c);
      window.HavenfallContext.workCoordinationV2WanderPatched = true;
    }
  }
  ...
  window.GameSystems?.registerTaskHandler?.('gather', 'work-coordination-v2.gather', handleGather, { order: 1 });
  window.GameSystems?.registerAutoTaskProvider?.('work-coordination-v2.marked-gather', assignMarkedResource, { order: 1 });
  window.GameSystems?.registerTick?.('work-coordination-v2', tick, { order: 21 });
  installOverrides();
})();