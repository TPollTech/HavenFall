...
(() => {
  if (window.HavenfallContext?.escapeActionRouterV2Installed) return;
  window.HavenfallContext = window.HavenfallContext || {};
  window.HavenfallContext.escapeActionRouterV2Installed = true;
  ...
  function cancelManualMode() {
    if (window.HavenfallManualControlV2?.isManualActive?.()) {
      window.HavenfallManualControlV2.setManualMode(false);
      return true;
    }
    if (state?.controlMode === 'manual' && window.HavenfallManualControl?.setMode) {
      window.HavenfallManualControl.setMode('auto');
      return true;
    }
    return false;
  }
  ...
  window.HavenfallEscapeRouterV2 = { cancelOneLayer };
})();