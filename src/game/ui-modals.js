...
function equipmentModalRows(c) {
  ...
  const rows = ['tool','weapon','offhand'].map(slot => {
    const key = eq[slot];
  ...
  modal.innerHTML = `
    <article class="colonist-modal-card">
      <header class="colonist-modal-header">
        <div>
          <div class="kicker">Detalhes do colono</div>
          <h3>${escapeHtml(c.name)} <span class="muted">${escapeHtml(c.role)}</span></h3>
          <p class="empty">${escapeHtml(c.note || 'Ocioso')}</p>
        </div>
      ...
      <h4>Estoque utilizável</h4>
      <div class="inventory-list">${colonyInventoryRows(c)}</div>
    </article>
  `;
