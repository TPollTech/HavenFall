...
(() => {
  if (window.HavenfallContext?.dockPanelStateFixInstalled) return;
  ...
  function patchRefreshDockPanel() {
    if (!window.HavenfallUI?.refreshDockPanel || window.HavenfallContext.dockPanelRefreshPatched) return;
    ...
    window.HavenfallContext.dockPanelRefreshPatched = true;
  }
  ...
  window.GameSystems?.registerTick?.('ui.dock-panel-state-fix', () => {
    clearClosedDockState();
    patchRefreshDockPanel();
  }, { order: 98 });

  patchRefreshDockPanel();
  window.HavenfallContext.dockPanelStateFixInstalled = true;
})();