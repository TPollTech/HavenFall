...
async function main() {
  ...
  try {
    ...
    const summary = await page.evaluate(() => {
      ...
      const cameraBefore = { x: window.Havenfall?.camera?.x, y: window.Havenfall?.camera?.y };
      return {
    ...
    if (!summary.phaserActive || !summary.hasLayer || !summary.hasCanvas || !summary.stats?.activeTiles) {
      throw new Error(`Phaser V1 nao ficou ativo corretamente: ${JSON.stringify(summary)}`);
    }
