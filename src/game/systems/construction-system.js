...
(() => {
  ...
  window.BuildAssetConfig = window.BuildAssetConfig || Object.freeze({
  ...
  function tileToWorldGrid(x, y, anchor = 'center') {
    const s = tileSize();
    const left = Math.round(Number(x) || 0) * s;
    const top = Math.round(Number(y) || 0) * s;
    if (anchor === 'top-left' || anchor === 'topLeft') return { x: left, y: top };
    if (anchor === 'bottom-center' || anchor === 'bottomCenter') return { x: left + s / 2, y: top + s };
    return { x: left + s / 2, y: top + s / 2 };
  }

  function worldToTileGrid(worldX, worldY) {
    const s = tileSize();
    return { x: Math.floor((Number(worldX) || 0) / s), y: Math.floor((Number(worldY) || 0) / s) };
  }

  function canvasClientToWorldGrid(clientX, clientY) {
    if (!canvas || !viewTransform) return null;
    const rect = canvas.getBoundingClientRect();
    if (!rect.width || !rect.height) return null;
    const px = (clientX - rect.left) * (canvas.width / rect.width);
    const py = (clientY - rect.top) * (canvas.height / rect.height);
    return { x: (px - viewTransform.offsetX) / viewTransform.scale, y: (py - viewTransform.offsetY) / viewTransform.scale };
  }

  function tileFromPointer(event) {
    const world = canvasClientToWorldGrid(event.clientX, event.clientY);
    return world ? worldToTileGrid(world.x, world.y) : null;
  }
  ...
  function eachTile(start, end, fn) {
    const r = rectBetween(start, end);
    if (!r || typeof fn !== 'function') return 0;
    let count = 0;
    for (let y = r.minY; y <= r.maxY; y++) for (let x = r.minX; x <= r.maxX; x++) fn({ x, y }, count++);
    return count;
  }
  ...
  function isWallAnchorAt(x, y) {
    ...
    return obj?.type === 'wall' || (obj?.type === 'blueprint' && typeForBuild(obj.buildType) === 'wall');
  }
  ...
  function occupiedByColonist(x, y) {
  ...
  function buildCostText(key) {
    const def = buildDefs?.[key];
    if (!def) return 'construção inválida';
    return typeof itemCostText === 'function' ? itemCostText(def.cost || {}, def.itemCost || {}) : Object.entries(def.cost || {}).map(([k, v]) => `${v} ${k}`).join(' + ');
  }

  function hasPayment(key) {
    const def = buildDefs?.[key];
    return !!def && (!def.cost || hasCost(def.cost)) && (!def.itemCost || hasItems(def.itemCost));
  }

  function applyBuildMetadata(obj, def) {
    if (!obj || !def) return obj;
    if (def.type === 'wall') {
      const cfg = WALL_CONFIG[def.wallMaterial || materialForBuild(obj.buildType || 'wall')] || WALL_CONFIG.wood;
  ...
  function placeBlueprint(key, x, y) {
    const result = queueConstruction(key, x, y);
    if (!result.ok) { if (typeof log === 'function') log(result.reason); return false; }
  ...
  function placeRect(key, start, end) {
    let placed = 0, attempted = 0, first = null, fail = null;
    eachTile(start, end, tile => {
      attempted++;
      const result = queueConstruction(key, tile.x, tile.y, { silent: true });
      if (result.ok) { placed++; first ||= result.blueprint; } else fail ||= result.reason;
    });
    if (placed) {
      if (typeof log === 'function') log(`${placed}/${attempted} planta${placed > 1 ? 's' : ''} enfileirada${placed > 1 ? 's' : ''}.`);
      const c = typeof selectedColonist === 'function' ? selectedColonist() : null;
      if (c && first && typeof assignBuild === 'function') assignBuild(c, first);
    } else if (fail && typeof log === 'function') log(fail);
    if (typeof updateUI === 'function') updateUI(true);
    return placed;
  }
  ...
  function routePrimaryObjectAction(c, obj) {
    if (obj?.type === 'door') { toggleDoorState(obj); return; }
  ...
  function installBuildButtons() {
    ...
    [{ key: 'wall_stone', html: 'Parede Pedra<br><small>6 pedra</small>' }, { key: 'wall_metal', html: 'Parede Metal<br><small>2 pedra + 6 metal</small>' }].reverse().forEach(item => {
  ...
  function buildButtonCapture(event) {
    const build = event.target?.closest?.('[data-build]');
    if (!build || !build.closest('#hud, #bottomActionBar, #buildPanel')) return;
  ...
  function mouseDown(event) {
    ...
    if (!tile) return;
    drag = { start: tile, current: tile, active: false, x: event.clientX, y: event.clientY };
    try { mouseTile = tile; } catch (_) {}
  }

  function mouseMove(event) {
    if (!drag || !currentBuild) return;
    const tile = tileFromPointer(event);
    if (!tile) return;
    drag.current = tile;
    try { mouseTile = tile; } catch (_) {}
    if (tile.x !== drag.start.x || tile.y !== drag.start.y || Math.hypot(event.clientX - drag.x, event.clientY - drag.y) > 5) drag.active = true;
  }

  function mouseUp(event) {
    if (event.button !== 0 || !drag || !currentBuild) return;
    ...
    placeRect(currentBuild, selection.start, selection.current);
  }
  ...
  function updateUIWrapper(force = false) {
    if (originalUpdateUI) originalUpdateUI(force);
    installBuildButtons();
    if (typeof dom !== 'undefined' && dom?.buildStatus) dom.buildStatus.textContent = currentBuild ? `Construindo: ${buildDefs[currentBuild]?.label || currentBuild}. Clique ou arraste.` : 'Nenhuma construção selecionada.';
    document.querySelectorAll('[data-build]').forEach(btn => {
      const key = btn.dataset.build;
      if (!buildDefs?.[key]) return;
      btn.classList.toggle('active', key === currentBuild);
      btn.title = `${buildDefs[key].label} · ${buildCostText(key)}`;
    });
  }
  ...
  function install() {
    ...
    installDefinitions();
    window.tileToWorld = tileToWorldGrid;
    window.worldToTile = worldToTileGrid;
    window.canvasClientToWorld = canvasClientToWorldGrid;
    window.tileFromPointerEvent = tileFromPointer;
    window.tileRectBetween = rectBetween;
    window.forEachTileInRect = eachTile;
    window.canPlaceBuild = canPlaceBuild;
    window.queueConstruction = queueConstruction;
    window.QueueConstruction = queueConstruction;
    window.placeBlueprintRect = placeRect;
    window.toggleDoorState = toggleDoorState;
    window.applyCompletedBuildMetadata = applyBuildMetadata;
    window.hasAdjacentWallForDoor = hasAdjacentWallForDoor;
    window.isWallAnchorAt = isWallAnchorAt;
    try { tileFromEvent = tileFromPointer; } catch (_) {}
    try { canPlace = (type, x, y) => canPlaceBuild(type === 'wall' ? 'wall' : type, x, y); } catch (_) {}
    try { placeBlueprint = placeBlueprint; } catch (_) {}
    try { routePrimaryObjectAction = routePrimaryObjectAction; } catch (_) {}
    try { updateUI = updateUIWrapper; } catch (_) {}
    canvas?.addEventListener('mousedown', mouseDown);
    canvas?.addEventListener('mousemove', mouseMove);
    canvas?.addEventListener('mouseup', mouseUp);
    canvas?.addEventListener('mouseleave', () => { if (drag && !drag.active) drag = null; });
    document.addEventListener('click', buildButtonCapture, true);
