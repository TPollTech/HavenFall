  ...
  dom.colonistCards.innerHTML = colonistCandidates.map((c, i) => `
    <article class="colonist-card ${c.locked ? 'locked' : ''}">
      <div class="colonist-head">
        ...
        <div><h2>${escapeHtml(c.name)}, ${c.age}</h2><div class="empty">${escapeHtml(c.role)} · Prefere ${escapeHtml(workPreferenceLabel(c.workPreferenceId || c.workPreference))}</div></div>
      </div>
      ...
      <div class="card-actions"><button data-reroll-colonist="${i}" ${c.locked ? 'disabled' : ''}>Rerolar</button><button data-lock-colonist="${i}" class="${c.locked ? 'active' : 'secondary'}">${c.locked ? 'Travado' : 'Travar'}</button></div>
    </article>`).join('');
}
