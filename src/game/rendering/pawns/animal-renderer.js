...
(() => {
  ...
  function drawAntlers(x, y, dir, color, scale = 1, spread = 0) {
    ...
    for (let index = 0; index < 2; index++) {
      ...
      const reach = 5.9 + spread * 1.4 + index * 0.5;
      const topReach = 7.8 + spread * 1.25 + index * 0.45;
  ...
  function drawDeerPawn(mob) {
    ...
    const bodyTall = v.bodyTall || 1;
    const neckX = x + f.x * 11.1 * s;
    const neckY = y - 7.1 * s + headLift * 0.26 * s;
    const headX = x + f.x * 18.5 * s;
    const headY = y - 6.4 * s + headLift * 0.38 * s;
    const muzzleX = x + f.x * 23 * s;
    ...
    drawOrganicBlob(neckX, neckY, 3.2 * s, 8.6 * s, p.neck || p.bodyLight, profile.outline, 1.05, {
    ...
    drawOrganicBlob(headX, headY, 5.1 * s, 3.8 * s, p.head || p.bodyLight, profile.outline, 1.2, {
    ...
    drawSnout(muzzleX, headY + 0.35 * s, 2.8 * s, 1.7 * s, p.snout || p.chest, profile.outline, p.nose);
    drawEllipse(muzzleX - f.x * 0.9 * s, headY + 1.3 * s, 2.1 * s, 0.95 * s, p.chest, null, 0, f.x * 0.05);
    drawTriangle([
      { x: headX + f.x * 0.6 * s, y: headY - 2.7 * s },
      { x: headX - f.x * 1.8 * s, y: headY - 8.2 * s + v.earTilt * 1.1 * s },
      { x: headX - f.x * 0.8 * s, y: headY - 3.1 * s }
    ], p.ear || p.head || p.bodyLight, profile.outline);
    drawTriangle([
      { x: headX - f.x * 0.6 * s, y: headY - 2.3 * s },
      { x: headX - f.x * 3.2 * s, y: headY - 7 * s + v.earTilt * 0.8 * s },
      { x: headX - f.x * 2.2 * s, y: headY - 3 * s }
    ], shade(p.ear || p.head || p.bodyLight, -10), profile.outline);
    drawAntlers(headX - f.x * 1.6 * s, headY - 4.8 * s, f.x, p.antler, s, v.earSpread || 0);
    drawAnimalEye(headX + f.x * 1.2 * s, headY - 0.55 * s, 1 * s, p.eye);
