...
(() => {
  ...
  function mountainCenters(world, seed) {
    ...
    const centers = [];
    for (let i = 0; i < centerCount(world); i++) {
  ...
  function mountainScore(x, y, centers) {
    let best = 0;
    for (const center of centers) {
      const dx = (x - center.x) / Math.max(1, center.rx);
      const dy = (y - center.y) / Math.max(1, center.ry);
      best = Math.max(best, Math.max(0, 1 - dx * dx - dy * dy) * center.power);
    }
    return best;
  }
  ...
  function countSolid(layer, x, y, radius = 1) {
    ...
    for (let yy = y - radius; yy <= y + radius; yy++) {
      for (let xx = x - radius; xx <= x + radius; xx++) {
        if ((xx !== x || yy !== y) && layer[yy]?.[xx]?.solid) count++;
      }
    }
    return count;
  }
  ...
  function pruneWeakEdges(layer) {
    ...
    const cols = layer[0]?.length || 0;
    const remove = [];
    for (let y = 1; y < rows - 1; y++) for (let x = 1; x < cols - 1; x++) if (layer[y][x]?.solid && countSolid(layer, x, y, 1) <= 1) remove.push([x, y]);
    for (const [x, y] of remove) layer[y][x] = null;
  }

  function pruneSmallClusters(layer, minSize = 18) {
    const rows = layer.length;
    const cols = layer[0]?.length || 0;
    const seen = new Set();
    const key = (x, y) => `${x},${y}`;
  ...
  function carveSpawn(layer, world) {
    ...
    for (let y = world.spawn.y - 8; y <= world.spawn.y + 8; y++) for (let x = world.spawn.x - 8; x <= world.spawn.x + 8; x++) if (Math.hypot(x - world.spawn.x, y - world.spawn.y) <= 8 && layer[y]?.[x]) layer[y][x] = null;
  }
  ...
  function makeDenseLayer(world) {
    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        if (x < 2 || y < 2 || x >= cols - 2 || y >= rows - 2 || nearSpawn(world, x, y)) continue;
        const terrain = world.terrain?.[y]?.[x] || 'grass';
        if (terrain === 'water' || terrain === 'sand') continue;
        const score = mountainScore(x, y, centers);
        const local = noise(seed, x, y, 'mountain-local-fill');
        const threshold = 0.20 - score * 0.26 - (terrain === 'stone' ? 0.18 : 0);
        if ((score > 0.09 && local > threshold) || (terrain === 'stone' && score > 0.03 && local > 0.22)) layer[y][x] = rockTile(world, x, y, seed);
      }
    }
  ...
  function hasSolidNear(layer, x, y, radius = 1) {
    for (let yy = y - radius; yy <= y + radius; yy++) for (let xx = x - radius; xx <= x + radius; xx++) if (hasSolidRock(layer, xx, yy)) return true;
    return false;
  }
  ...
  window.HavenfallGeologyMassSystem = { version: VERSION, applyDenseGeology, cleanupVegetationOnGeology, purgeLooseResourcesOnGeology, objectInvalidForLayer };
  window.HavenfallGeologyMassPatch = window.HavenfallGeologyMassSystem;
  window.GameSystems?.registerTick?.('geology.mass-system', updateGeologyMassSystem, { order: 11 });
})();
